...
import { normalizeInternalLinkQuery, stripQueryType } from './queryModel';

describe('normalizeInternalLinkQuery', () => {
  const traceId = '75d7a6e5c07de26e0238cd17a281a190';

  it('converts explicit trace links with bare trace ids into trace queries', () => {
    const query = {
      refId: 'A',
      query: traceId,
      queryType: 'traces',
  ...
  it('does not let a stale queryType override explicit trace search metrics', () => {
    const query = {
      refId: 'A',
      query: `trace_id:${traceId}`,
      queryType: 'traces',
      metrics: [{ id: '3', type: 'trace_search', settings: { limit: '20', spanLimit: '1000' } }],
      bucketAggs: [],
      filters: [],
    } as ElasticsearchQuery & { queryType: 'traces' };

    expect(normalizeInternalLinkQuery(query)).toEqual({
      refId: 'A',
      query: `trace_id:${traceId}`,
      metrics: [{ id: '3', type: 'trace_search', settings: { limit: '20', spanLimit: '1000' } }],
      bucketAggs: [],
  ...
  it('strips transient queryType markers from editor updates', () => {
    const query = {
      refId: 'A',
      query: `trace_id:${traceId}`,
      queryType: 'traces',
    ...
    expect(stripQueryType(query)).toEqual({
      refId: 'A',
      query: `trace_id:${traceId}`,
      metrics: [{ id: '3', type: 'logs', settings: { limit: '100' } }],
      bucketAggs: [],
  ...
  it('leaves trace_id log filters unchanged', () => {
    const query: ElasticsearchQuery = {
      refId: 'A',
      query: `trace_id:${traceId}`,
      metrics: [{ id: '3', type: 'logs', settings: { limit: '100' } }],
  ...
  it('leaves bare trace ids unchanged when logs are explicitly selected', () => {
    const query: ElasticsearchQuery = {
      refId: 'A',
      query: traceId,
      metrics: [{ id: '3', type: 'logs', settings: { limit: '100' } }],
